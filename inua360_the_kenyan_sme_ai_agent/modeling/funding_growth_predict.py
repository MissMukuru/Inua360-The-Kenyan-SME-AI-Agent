...
@app.command()
def predict(
    model_path: Path = MODELS_DIR / "best_funding_model.pkl",
    single_input: str = None,
):
    """Predict funding status for a single SME from JSON input."""
    ...
    X["predicted_funding_status"] = predictions
    logger.success("Predictions generated successfully.")

    output_path = PREDICTIONS_DIR / "predicted_funding.json"
    X[['predicted_funding_status']].to_json(output_path, orient='records', indent=4)
    logger.success(f"Predictions saved to {output_path}")

    print(json.dumps(X[['predicted_funding_status']].to_dict(orient='records'), indent=4))
